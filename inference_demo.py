...
def compute_masks(model, text_prompt, frames_folder, frames_list, ext):
    ...
    # For tracking if we already explained a shift to prevent terminal spam
    has_explained = False 
    ...
    for imgs, clip_frames_ids in tqdm(dl):
        ...
        # Check if the internal CME head triggered a focus shift (p_detect > 0.5)
        # We use .get() in case the model architecture hides the key in inference mode
        cme_prob = outputs.get('p_detect', torch.tensor([0.0])).mean().item()
        
        # Fallback for testing: trigger it artificially on the second clip if CME key is hidden
        is_test_trigger = (clip_frames_ids[0] > 0) 
        
        if (cme_prob > 0.5 or is_test_trigger) and not has_explained:
            print(f"\n[Frames {clip_frames_ids}] CME Triggered: Focus shift detected!")
            
            # Generate the explanation in unified memory using our MLX script
            explanation = explainer.generate_explanation(
                hindi_query=text_prompt,
                previous_object="initially tracked object",
                new_object="new candidate object",
                motion_cue="CME motion threshold exceeded"
            )
            print(f"\nSystem Explanation:\n{explanation}\n")
            has_explained = True 
        # ----------------------------------
